# This Makefile is for the Backup::Datto extension to perl.
#
...
#     ABSTRACT => q[Collection of modules to report data from Datto backup devices.]
...
#     NAME => q[Backup::Datto]
#     PL_FILES => {  }
#     PREREQ_PM => { XML::SIMPLE=>q[0], Time::Piece=>q[1.15], Time::Seconds=>q[1.15], Time::Interval=>q[1.22], LWP=>q[0], Test::More=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Backup/Datto/Collection.pm]
#     clean => { FILES=>q[Backup::Datto-*] }
...
INST_LIBDIR      = $(INST_LIB)\Backup
INST_ARCHLIBDIR  = $(INST_ARCHLIB)\Backup
...
TO_INST_PM = lib/Backup/Datto/Agent.pm \
	lib/Backup/Datto/Collection.pm \
	lib/Backup/Datto/Device.pm

PM_TO_BLIB = lib/Backup/Datto/Device.pm \
	blib\lib\Backup\Datto\Device.pm \
	lib/Backup/Datto/Agent.pm \
	blib\lib\Backup\Datto\Agent.pm \
	lib/Backup/Datto/Collection.pm \
	blib\lib\Backup\Datto\Collection.pm
...
DISTNAME = Backup-Datto
DISTVNAME = Backup-Datto-0.1.1
...
Datto.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"Backup::Datto\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) "abstract: Collection of modules to report data from Datto backup devices." >> META_new.yml
	$(NOECHO) $(ECHO) author: >> META_new.yml
	$(NOECHO) $(ECHO) "  - 'Matthew Topper <topperm9@gmail.com>'" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "name: Backup-Datto" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "  Time::Interval: 1.22" >> META_new.yml
	$(NOECHO) $(ECHO) "  Time::Piece: 1.15" >> META_new.yml
	$(NOECHO) $(ECHO) "  Time::Seconds: 1.15" >> META_new.yml
	$(NOECHO) $(ECHO) "  XML::SIMPLE: 0" >> META_new.yml
	$(NOECHO) $(ECHO) "  perl: 5.006" >> META_new.yml
	$(NOECHO) $(ECHO) "version: v0.1.1" >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) {{ > META_new.json
	$(NOECHO) $(ECHO) "   \"abstract\" : \"Collection of modules to report data from Datto backup devices.\"," >> META_new.json
	$(NOECHO) $(ECHO) "   \"author\" : [" >> META_new.json
	$(NOECHO) $(ECHO) "      \"Matthew Topper ^<topperm9^@gmail.com^>\"" >> META_new.json
	$(NOECHO) $(ECHO) "   ]," >> META_new.json
	...
	$(NOECHO) $(ECHO) "   }}," >> META_new.json
	$(NOECHO) $(ECHO) "   \"name\" : \"Backup-Datto\"," >> META_new.json
	...
	$(NOECHO) $(ECHO) "         \"requires\" : {{" >> META_new.json
	$(NOECHO) $(ECHO) "            \"LWP\" : \"0\"," >> META_new.json
	$(NOECHO) $(ECHO) "            \"Time::Interval\" : \"1.22\"," >> META_new.json
	$(NOECHO) $(ECHO) "            \"Time::Piece\" : \"1.15\"," >> META_new.json
	$(NOECHO) $(ECHO) "            \"Time::Seconds\" : \"1.15\"," >> META_new.json
	...
	$(NOECHO) $(ECHO) "   \"version\" : \"v0.1.1\"" >> META_new.json
	$(NOECHO) $(ECHO) }} >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) "    <ABSTRACT>Collection of modules to report data from Datto backup devices.</ABSTRACT>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <AUTHOR>Matthew Topper &lt;topperm9@gmail.com&gt;</AUTHOR>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <IMPLEMENTATION>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <PERLCORE VERSION=\"5,006,0,0\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"LWP::\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE VERSION=\"1.22\" NAME=\"Time::Interval\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE VERSION=\"1.15\" NAME=\"Time::Piece\" />" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Time::Seconds\" VERSION=\"1.15\" />" >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e "pm_to_blib({{@ARGV}}, '$(INST_LIB)\auto', q[$(PM_FILTER)], '$(PERM_DIR)')" -- \
	  lib/Backup/Datto/Device.pm blib\lib\Backup\Datto\Device.pm \
	  lib/Backup/Datto/Agent.pm blib\lib\Backup\Datto\Agent.pm \
	  lib/Backup/Datto/Collection.pm blib\lib\Backup\Datto\Collection.pm 
